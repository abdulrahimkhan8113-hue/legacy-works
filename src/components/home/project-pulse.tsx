...
export function ProjectPulse() {
  ...
  const [isMounted, setIsMounted] = useState(false); // Fix 1: Client mount tracking

  // Ensure animations do not run during server-side building or optimization passes
  useEffect(() => {
    setIsMounted(true);
  }, []);
  ...
  return (
    <section className="relative overflow-x-hidden py-12 sm:py-20 lg:py-28">
      <div className="absolute inset-0 grid-blueprint opacity-50" />
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        
        <div className="mx-auto max-w-2xl text-center">
          <div className="text-xs font-semibold uppercase tracking-[0.2em] sm:tracking-[0.3em] text-copper">
            Project Pulse
          </div>
        ...
        <div className="mt-12 grid items-center gap-10 lg:grid-cols-12 lg:gap-12">
          ...
          <div className="lg:col-span-7 w-full overflow-hidden">
            <div className="relative mx-auto aspect-[4/5] w-full max-w-full sm:max-w-xl">
              <svg
                viewBox="0 0 100 125"
          ...
          {/* DETAIL CARD COLUMN */}
          <div className="lg:col-span-5 w-full min-h-[420px] relative">
            {/* Fix 2: Wrap execution conditional block inside the AnimatePresence scope smoothly */}
            {isMounted && (
              <AnimatePresence mode="wait">
                <motion.div
                  key={hovered ?? "empty"} // Fix 3: Use simple numeric variable context for animation keys
                  initial={{ opacity: 0, y: 15 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -15 }}
                  transition={{ duration: 0.25, ease: "easeInOut" }}
                  className="glass-card overflow-hidden rounded-2xl w-full border border-copper/10 bg-card absolute inset-0"
                >
                  {activeProject ? (
                    <div className="w-full h-full flex flex-col">
                      <div className="relative h-56 w-full overflow-hidden sm:h-64 bg-muted">
                        <img 
